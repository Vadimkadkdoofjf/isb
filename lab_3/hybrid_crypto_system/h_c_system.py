...
from hybrid_crypto_system.symmetric.symmetric_system import Symmetric
...
class HybridCryptoSystem:
    ...
    def generate_keys(
        ...
        encrypted_symmetric_key, private_key, public_key = (
            Asymmetric.generate_keys(self.__key_length)
        )
    ...
    def encrypt_data(
        ...
        :return: None
        """
        if not (plain_text := FileWork.open_file(plain_text_dir, "rb")):
            raise ValueError("Text file must not be empty")
        if not (private_bytes := FileWork.open_file(private_key_dir, "rb")):
            raise ValueError("Private key must not be empty")
        if not (
            encrypted_symmetric_key := FileWork.open_file(
                encrypted_symmetric_key_dir, "rb"
            )
        ):
            raise ValueError("Encrypted symmetric key must not be empty")

        encrypted_data = Symmetric.encrypt_data(
    ...
    def decrypt_data(
        ...
        :return: None
        """
        if not (encrypted_text := FileWork.open_file(encrypted_text_dir, "rb")):
            raise ValueError("Text file must not be empty")
        if not (private_bytes := FileWork.open_file(private_key_dir, "rb")):
            raise ValueError("Private key must not be empty")
        if not (
        ...
        decrypted_data = Symmetric.decrypt_data(
            encrypted_text, private_bytes, encrypted_symmetric_key
        )
        FileWork.save(decrypted_text_dir, decrypted_data, "wb")